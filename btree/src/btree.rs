...
pub struct BTree<K, V, A: Allocator + Clone = Global> {
...
unsafe impl<#[may_dangle] K, #[may_dangle] V, A: Allocator + Clone> Drop for BTree<K, V, A> {
    fn drop(&mut self) {
...
impl<K, V, A: Allocator + Clone> core::panic::UnwindSafe for BTree<K, V, A>
where
...
impl<K: Clone, V: Clone, A: Allocator + Clone> Clone for BTree<K, V, A> {
    fn clone(&self) -> BTree<K, V, A> {
...
impl<K, V, A: Allocator + Clone> BTree<K, V, A> {
    ...
    /// ```
    #[must_use]
    pub const fn new_in(alloc: A) -> BTree<K, V, A> {
        BTree {
            root: None,
            length: 0,
            alloc: ManuallyDrop::new(alloc),
...
impl<K, V, A: Allocator + Clone> BTree<K, V, A> {
    pub fn find(&self, cmp: impl FnMut(&K) -> Ordering) -> Option<(&K, &V)> {
        let root_node = self.root.as_ref()?.reborrow();
        match root_node.search_tree_with(cmp) {
            Found(handle) => Some(handle.into_kv()),
            GoDown(_) => None,
        }
    }

    pub fn find_mut(&mut self, cmp: impl FnMut(&K) -> Ordering) -> Option<(&mut K, &mut V)> {
        let root_node = self.root.as_mut()?.borrow_mut();
        match root_node.search_tree_with(cmp) {
            Found(handle) => Some(handle.into_kv_mut()),
            GoDown(_) => None,
        }
    }

    pub fn entry(&mut self, cmp: impl FnMut(&K) -> Ordering) -> Entry<'_, K, V, A> {
        ...
        match map.root {
            ...
            Some(ref mut root) => match root.borrow_mut().search_tree_with(cmp) {
...
impl<'a, K, V, A: Allocator + Clone> IntoIterator for &'a BTree<K, V, A> {
    type Item = (&'a K, &'a V);
...
impl<'a, K, V, A: Allocator + Clone> IntoIterator for &'a mut BTree<K, V, A> {
...
impl<K, V, A: Allocator + Clone> IntoIterator for BTree<K, V, A> {
    type Item = (K, V);
...
pub struct ExtractIf<'a, K, V, R, F, A: Allocator + Clone = Global> {
    pred: F,
...
impl<K: Hash, V: Hash, A: Allocator + Clone> Hash for BTree<K, V, A> {
...
impl<K: PartialEq, V: PartialEq, A: Allocator + Clone> PartialEq for BTree<K, V, A> {
    fn eq(&self, other: &BTree<K, V, A>) -> bool {
        self.len() == other.len() && self.iter().zip(other).all(|(a, b)| a == b)
    }
}

impl<K: Eq, V: Eq, A: Allocator + Clone> Eq for BTree<K, V, A> {}

impl<K: PartialOrd, V: PartialOrd, A: Allocator + Clone> PartialOrd for BTree<K, V, A> {
    #[inline]
    fn partial_cmp(&self, other: &BTree<K, V, A>) -> Option<Ordering> {
        self.iter().partial_cmp(other.iter())
    }
}

impl<K: Debug, V: Debug, A: Allocator + Clone> Debug for BTree<K, V, A> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
